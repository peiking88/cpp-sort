...
TEST_CASE( "parallel_simd_sorter tests with projections",
           "[parallel_simd_sorter][projection]" )
{
    ...
    SECTION( "small dataset: sort with random-access range and projection" )
    {
        ...
        cppsort::parallel_simd_sort(vec, &wrapper::value);
    ...
    SECTION( "small dataset: sort with iterators and projection" )
    {
        ...
        cppsort::parallel_simd_sort(vec.begin(), vec.end(), &wrapper::value);
        CHECK( helpers::is_sorted(vec.begin(), vec.end(), std::less{}, &wrapper::value) );
    }
    ...
    SECTION( "small dataset: sort with compare and projection" )
    {
        ...
        cppsort::parallel_simd_sort(vec, std::greater{}, &wrapper::value);
    ...
    SECTION( "small dataset: sort with iterators, compare and projection" )
    {
        ...
        cppsort::parallel_simd_sort(vec.begin(), vec.end(), std::greater{}, &wrapper::value);
        CHECK( helpers::is_sorted(vec.begin(), vec.end(), std::greater{}, &wrapper::value) );
    }
}

TEST_CASE( "parallel_simd_sorter large dataset tests with projections",
           "[parallel_simd_sorter][projection][large]" )
{
    using wrapper = generic_wrapper<int>;
...
TEST_CASE( "parallel_simd_sorter tests with string projection",
           "[parallel_simd_sorter][projection][string]" )
{
    struct Person {
        std::string name;
        int age;
    ...
    SECTION( "small dataset: sort by name" )
    {
        ...
        cppsort::parallel_simd_sort(people, &Person::name);
    ...
    SECTION( "small dataset: sort by age" )
    {
        ...
        cppsort::parallel_simd_sort(people, &Person::age);
        CHECK( std::is_sorted(people.begin(), people.end(),
    ...
    SECTION( "small dataset: sort by age descending" )
    {
        ...
        cppsort::parallel_simd_sort(people, std::greater{}, &Person::age);
        CHECK( std::is_sorted(people.begin(), people.end(),
            [](const Person& a, const Person& b) { return a.age > b.age; }) );
    }
}
