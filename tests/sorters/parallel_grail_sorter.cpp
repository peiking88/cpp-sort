...
TEST_CASE( "parallel_grail_sorter tests", "[parallel_grail_sorter]" )
{
    SECTION( "empty collection" )
    {
        std::vector<int> vec;
        cppsort::parallel_grail_sort(vec);
        CHECK( vec.empty() );
    }
    ...
    {
        std::vector<int> vec = { 5, 3, 8, 1, 9, 4, 2, 7, 6, 0 };
        cppsort::parallel_grail_sort(vec, std::greater<>{});
...
TEST_CASE( "parallel_grail_sorter stability test", "[parallel_grail_sorter][stability]" )
{
    // Test that parallel_grail_sorter is stable
    struct Element {
        int value;
