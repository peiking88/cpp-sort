...
TEST_CASE( "parallel_heap_sorter with projection", "[parallel_heap_sorter][projection]" )
{
    SECTION( "projection with integers (negate)" )
    {
        std::vector<int> vec = { 5, 3, 8, 1, 9, 4, 2, 7, 6, 0 };
        // Sort by negative value (descending order)
        cppsort::parallel_heap_sort(vec, std::less<>{}, [](int n) { return -n; });
    ...
    SECTION( "projection with pairs (first element)" )
    {
        ...
        cppsort::parallel_heap_sort(vec, std::less<>{}, &std::pair<int, std::string>::first);
    ...
    SECTION( "projection with pairs (second element)" )
    {
        ...
        cppsort::parallel_heap_sort(vec, std::less<>{}, [](const auto& p) { return p.second; });
    ...
    SECTION( "projection with tuples" )
    {
        ...
        cppsort::parallel_heap_sort(vec, std::less<>{}, [](const auto& t) { return std::get<0>(t); });
    ...
    SECTION( "projection with struct" )
    {
        ...
        // Sort by age
        cppsort::parallel_heap_sort(vec, std::less<>{}, &Person::age);
    ...
    SECTION( "large dataset with projection" )
    {
        ...
        cppsort::parallel_heap_sort(vec, std::less<>{}, &Data::key);
